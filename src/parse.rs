...
impl Type {
    pub fn from(pair: Pair<Rule>) -> Self {
        ...
        let actual_type = match actual_pair.into_inner().next().unwrap().as_rule() {
            Rule::INT => Type::Int,
            Rule::BOOL => Type::Bool,
                        ...
                        arg.to_string()
                            + &{
                                ...
                                while let Some(arg) = it.next() {
                                    others = format!("{},{}", others, arg.to_string());
                                }
                                others
                            }
                    } else {
...
impl<'a> ExpNode<'a> {
    pub fn from(exp_pair: Pair<'a, Rule>) -> Self {
        match exp_pair.as_rule() {
            Rule::exp => ExpNode::from(exp_pair.into_inner().next().unwrap()),
            Rule::assignExp => AssignExp(AssignExpNode::from(exp_pair)),
            Rule::boolExp | Rule::logicTerm | Rule::compareExp | Rule::sum | Rule::term => {
            ...
            Rule::factor => {
                ...
                if let Rule::NOT = fst.as_rule() {
                    return UnaryExp(UnaryExpNode {
                        op: UnaryOp::Not,
                        exp: Box::new(ExpNode::from(inner_pairs.next().unwrap())),
                ...
                let actual_node: ExpNode = match actual.as_rule() {
                    Rule::INTLITERAL => IntLit(IntLitNode::from(actual.clone())),
                    Rule::SHORTLITERAL => ShortLit(ShortLitNode::from(actual.clone())),
                    Rule::STRLITERAL => StrLit(StrLitNode::from(actual.clone())),
                    Rule::TRUE => True(TrueNode::from(actual.clone())),
                    Rule::FALSE => False(FalseNode::from(actual.clone())),
                    Rule::LPAREN => ExpNode::from(inner_pairs.next().unwrap()),
                    Rule::AMP => {
...
impl Unparse for ReturnStmtNode<'_> {
    fn to_string(&self, depth: usize) -> String {
        match &self.exp {
            Some(exp) => format!("{}return {};", "    ".repeat(depth), exp.to_string(0),),
...
impl Unparse for ReadStmtNode<'_> {
    fn to_string(&self, depth: usize) -> String {
        format!("{}read {};", "    ".repeat(depth), self.lval.to_string(0),)
    }
}

impl Unparse for WriteStmtNode<'_> {
    fn to_string(&self, depth: usize) -> String {
        format!("{}write {};", "    ".repeat(depth), self.exp.to_string(0),)
    }
}
...
impl<'a> StmtNode<'a> {
    pub fn from(pair: Pair<'a, Rule>) -> Self {
        ...
        match fst.as_rule() {
            ...
            Rule::assignExp => AssignStmt(AssignStmtNode {
                exp: AssignExpNode::from(fst),
                pos: Pos::from(&pair),
            }),
            Rule::lval => match inner_pairs.next().unwrap().as_rule() {
                Rule::DEC => PostDecStmt(PostDecStmtNode {
                    lval: LValNode::from(fst),
                    pos: Pos::from(&pair),
                }),
                Rule::INC => PostIncStmt(PostIncStmtNode {
                    lval: LValNode::from(fst),
                    pos: Pos::from(&pair),
                }),
                _ => unreachable!(),
            },
            Rule::READ => ReadStmt(ReadStmtNode {
                lval: LValNode::from(inner_pairs.next().unwrap()),
                pos: Pos::from(&pair),
            }),
            Rule::WRITE => WriteStmt(WriteStmtNode {
                exp: ExpNode::from(inner_pairs.next().unwrap()),
                pos: Pos::from(&pair),
            }),
            Rule::IF => {
                let exp = ExpNode::from(inner_pairs.nth(1).unwrap());
                let true_stmts = generate_stmt_list(inner_pairs.nth(2).unwrap());
                let maybe_else = inner_pairs.nth(1);
                let pos = Pos::from(&pair);
                if let Some(_) = maybe_else {
                    let else_stmts = generate_stmt_list(inner_pairs.nth(1).unwrap());
            ...
            Rule::RETURN => {
                ...
                match maybe_exp.as_rule() {
                    Rule::exp => ReturnStmt(ReturnStmtNode {
                        exp: Some(ExpNode::from(maybe_exp)),
                        pos: Pos::from(&pair),
                    }),
                    _ => ReturnStmt(ReturnStmtNode {
                        exp: None,
                        pos: Pos::from(&pair),
                    }),
                }
            }
            Rule::callExp => CallStmt(CallStmtNode {
