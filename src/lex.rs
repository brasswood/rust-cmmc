    ...
    for token_pair in token_pairs {
        ...
        match rule {
            ...
            Rule::ERROR => {
                ...
                let (start_line, start_col) = error_token_pair.as_span().start_pos().line_col();
                let (end_line, end_col) = error_token_pair.as_span().end_pos().line_col();
                write!(
                    io::stderr(),
                    "FATAL [{},{}]-[{},{}]: ",
                    start_line,
                ...
                match error_token_pair.as_rule() {
                    Rule::UNTERM_BADESC_STRLIT => writeln!(
                        io::stderr(),
                        "Unterminated string literal with bad escape sequence ignored"
                    ),
                    Rule::BADESC_STRLIT => writeln!(
                        io::stderr(),
                        "String literal with bad escape sequence ignored"
                    ),
                    Rule::UNTERM_STRLIT => {
                        writeln!(io::stderr(), "Unterminated string literal ignored")
                    }
                    Rule::ILLEGAL_CHAR => writeln!(
                        io::stderr(),
                        "Illegal character {}",
                        error_token_pair.as_str()
                    ),
                    _ => unreachable!(),
                }
                .unwrap();
    ...
    } = start_token
    {
        ...
        match writeln!(buf, "{} [{},{}]", &tokenspec, &line, &col) {
            ...
            Err(_) => {
                writeln!(
                    io::stderr(),
                    "Error writing token {} [{},{}] to output file",
                    &tokenspec,
                    &line,
                    &col
                )
